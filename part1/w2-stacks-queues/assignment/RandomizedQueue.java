...
public class RandomizedQueue<Item> implements Iterable<Item> {
    ...
    public Item dequeue() {
        ...
        int remove_index = new Random().nextInt(N);

        Item item = q[remove_index + first];
        q[remove_index + first] = null;                            // to avoid loitering
        
        for (int i = remove_index; i > first; i--) {
        	q[i] = q[i - 1];
        }
     ...
     * @return a random item in the queue
     * @throws java.util.NoSuchEementException if this queue is empty
     */
    public Item sample() {
    	if (isEmpty()) throw new NoSuchElementException("Queue underflow");
    	int remove_index = new Random().nextInt(N);
